...
def inference(args):
    ...
    ratings = torch.tensor(()).cpu() #empty tensor
    with torch.no_grad():
        ...
        for idx, batch in pbar:
            ...
            #output_best10 = np.argpartition(output, -10)[-10:] #[B,10]
            #user_list, item_list = dataset.decode_offset(x.cpu())

            preds = torch.concat((x,output),dim =1) # [B , 4]
    ...
    for user_id in range(n_users):
        idx = np.where(outputs[0] == user_id)
        user_rating = outputs[idx]
        ...
        info.append([user,item] for item in movie_list)
