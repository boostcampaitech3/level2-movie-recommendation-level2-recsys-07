...
class Trainer:
    ...
    def predict_full(self, seq_out):
        ...
        # print('seq_out.shape',seq_out.shape) #[256, 64]
        # [[ 0.6877, -1.1246, -0.2123,  ...,  0.3668,  0.4744,  0.8680],
        # [ 0.9977, -1.9076, -0.1637,  ...,  0.6091,  0.4646,  1.0975],
        # print('test_item_emb.transpose(0, 1).shape',test_item_emb.transpose(0, 1).shape) #[64, 119147]
        #[[-0.0104,  0.1869, -0.0866,  ..., -0.0401, -0.4460,  0.0103],
        # [-0.0104, -0.3584, -0.5814,  ...,  0.2343,  0.0150,  0.0570],

        # rating_pred = torch.matmul(seq_out, test_item_emb.transpose(0, 1))
        # print('rating_pred1',rating_pred.shape,type(rating_pred)) #torch.Size([256, 119147]) <class 'torch.Tensor'>
    
        seq_out=seq_out.detach().cpu().numpy()
        test_item_emb=test_item_emb.detach().cpu().numpy()
        
        rating_pred = euclidean_distances(seq_out,test_item_emb) # torch.matmul->euclidean
        rating_pred=torch.tensor(rating_pred)
        # print('rating_pred2',rating_pred.shape,type(rating_pred)) #rating_pred2 (256, 119147) <class 'numpy.ndarray'> -> torch.Size([256, 119147]) <class 'torch.Tensor'>
        return rating_pred
